# RDPLauncher
Запуск файлов *.RPD для офисных работников, которые имеют основной и резервный каналы связи. 
Основная идея избавить запускать подключения по очереди при сбоях. Программа при запуске проверяет IP:PORT и отмечает рабочий, возможно несколько конфигураций сотрудников. Программа хранит в настройках только IP:PORT в "config.ini", никаких паролей не запоминает. Для работы требуется .NET 8, отдельно есть версия со встроенным .NET 8 для запуска без установки.

Пример конфигурации:

[Options]

CloseAfterLaunch=True # действие программы при подключении false - не закрываться | true - закрыться 
 
[Servers]

MainIP=0.0.0.0 #адрес основной

MainPort=3369 #порт основной
 
BackupIP=0.0.0.0 #адрес резервный

BackupPort=3369 #порт резервный
 
[Profiles] 

Бухгалтер=buh.rdp|buh_rezerv.rdp #имена файлов для запуска

Кассир=kassir.rdp|kassir_rezerv.rdp #имена файлов для запуска

Менеджер=manager.rdp|manager_rezerv.rdp #имена файлов для запуска
